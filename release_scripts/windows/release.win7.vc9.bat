...
set PROJECT_NAME=OpenBoard
set APPLICATION_NAME=PROINTEK Canvas
@SET QT_DIR=
FOR /F %%I IN ('qmake -query QT_INSTALL_PREFIX') DO SET "QT_DIR=%%I"
echo QT_DIR %QT_DIR%
...
dir "%IMPORTER_PATH%"
IF NOT EXIST "%IMPORTER_PATH%" (
	echo Eror: IMPORTER_PATH %IMPORTER_PATH%
...
nmake release
IF NOT EXIST release\"%IMPORTER_NAME%.exe" (
	echo Eror: IMPORTER_NAME %IMPORTER_NAME%
	GOTO EXIT_WITH_ERROR
)
cd %HOME_DIR%
...
"%QT_BIN%\qmake.exe" %PROJECT_NAME%.pro

call "%LRELEASE%" "%PROJECT_NAME%.pro"
...
IF NOT EXIST "build\win32\release\product\%APPLICATION_NAME%.exe" (
	echo Eror: APPLICATION_NAME %APPLICATION_NAME%
	GOTO EXIT_WITH_ERROR
)
...
call "%INNO_EXE%" "%SCRIPT_PATH%\%PROJECT_NAME%.iss" /F"%APPLICATION_NAME%_Installer_%VERSION%"
